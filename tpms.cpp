...
byte crc8(byte* out, int len) {
  byte crc=0x80;
  for(int x=0;x<len;x++)
    crc=pgm_read_byte_near(crctable2+((*(out++) ^ crc) & 0xff));
  return crc;
}
...
void decode(byte* inp, byte* realtemp, float* pressure, uint32_t* id, byte* state) {
    for(int x=0;x<4;x++)
      ((byte*)id)[3-x]=inp[x];
    *state = (inp[4] & 0x80) | (inp[6] & 0x7f); // status bit and 0 filler
    byte pressure1 = (inp[4] & 0x7f) << 1 | inp[5] >> 7, pressure2 = inp[7] ^ 0xff;
    *realtemp = (int)((inp[5] & 0x7f) << 1 | inp[6] >> 7)-40;
    if(pressure1!=pressure2)
      pressure1=0;
    *pressure=((((float)pressure1)*0.25)-7.)*0.137895;
}

void encode(byte* out, byte realtemp, float pressure, uint32_t id, byte state) {
    byte pres=(byte)((pressure*14.5038+7.)*4.+.5),
    temp=(byte)(realtemp+40.);
    for(int x=0;x<4;x++)
      out[x]=((byte*)&id)[3-x];
    out[4]=(pres >> 1) | (state & 0x80);
    out[5]=((pres & 1) << 7) | (temp >> 1);
    out[6]=(state & 0x7f) | ((temp & 1) << 7);
    out[7]=pres^0xff;
    out[8]=crc8(out,8);
}
